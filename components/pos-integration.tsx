...
import { useInventoryOperations } from "@/lib/hooks/useInventory";
...
export default function POSIntegration({ onNavigate }: POSIntegrationProps) {
  // Screen navigation
  ...
  const { customers, updateCreditBalance, recordPaymentCollection } =
    useCustomers();
  const { processInventoryChange } = useInventoryOperations();

  // Transform database products for POS
  ...
  // Transform customers
  const enhancedCustomers: Customer[] = customers.map((customer) => ({
    id: customer.id,
    name: customer.name,
    phone_number: customer.phone_number,
    email: customer.email,
    customer_type: "retail", // Default, can be enhanced
  ...
  const addToCart = (product: any, selectedUnit?: any) => {
    ...
    const existingItem = cart.find(
      (item) => item.id === product.id && item.unit_name === unit.name,
    );
  ...
  const updateCartQuantity = (
    ...
    if (quantity <= 0) {
      setCart((prev) =>
        prev.filter(
          (item) => !(item.id === productId && item.unit_name === unitName),
        ),
      );
    } else {
      setCart((prev) =>
  ...
  const handlePaymentComplete = async (paymentData: any) => {
    try {
      ...
      for (const item of cart) {
        ...
        if (product) {
          ...
          await processInventoryChange(
            item.id,
            item.unit_name, // Unit ID (simplified)
            ...
            `Sale to ${selectedCustomer?.name || "Walk-in customer"}`,
            "System",
          );
        }
      }
      ...
      if (paymentData.payment_method === "credit" && selectedCustomer) {
        const newBalance = (selectedCustomer.current_balance || 0) + cartTotal;
        await updateCreditBalance(selectedCustomer.id, newBalance);
      }

      // Handle split payments with credit
      if (paymentData.split_payments && selectedCustomer) {
        const creditPayment = paymentData.split_payments.find(
          (p: any) => p.method === "credit",
        );
        if (creditPayment) {
          const newBalance =
            (selectedCustomer.current_balance || 0) + creditPayment.amount;
          await updateCreditBalance(selectedCustomer.id, newBalance);
        }
      }
        ...
        <div className="max-w-6xl mx-auto p-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            ...
            <Card className="cursor-pointer hover:shadow-lg transition-shadow">
              <CardContent className="pt-6">
                <div className="text-center">
                  <BarChart3 className="w-12 h-12 text-indigo-600 mx-auto mb-3" />
                          ...
                          <CardContent className="p-3">
                            <div className="flex justify-between items-start mb-2">
                              ...
                              <Button
                                ...
                                size="sm"
                                onClick={() =>
                                  updateCartQuantity(item.id, item.unit_name, 0)
                                }
                              >
        ...
        {showBarcodeScanner && (
          <BarcodeScanner
            ...
            products={products.map((p) => ({
              ...
              name: p.name,
              sku: p.sku,
              barcode: p.sku, // Using SKU as barcode for demo
              price: p.price,
              stock: p.stock,
              image_url: p.image,
              category: p.category,
